...
class StreamRole:
    ...
    if ctx.invoked_subcommand is None:
      ...
      msg = box(
        ('Enabled: {}\n'
         ...
         '').format(
           enabled, role and role.name, promote,
           promote_from and promote_from.name, lax_promote
        ),
  ...
  async def streamroleset_toggle(self, ctx: commands.Context, on_off: bool = None):
    ...
    if streaming_role is None and target_state:
      await ctx.send(
        ('You need to set a role with `{}streamroleset role` '
         'before you can enable StreamRole.'
         '').format(ctx.prefix)
      )
      return
  ...
  async def streamroleset_role(self, ctx: commands.Context, *, role: discord.Role):
    ...
    await ctx.send(
      ('Done. Members who are streaming will now be given the role `{}`. '
       'Ensure you also turn on StreamRole with `{}streamroleset toggle`.'
  ...
  async def streamroleset_promote_toggle(self, ctx: commands.Context, on_off: bool = None):
    ...
    if prereq_role is None and target_state:
      await ctx.send(
        ('You need to set a role with `{}streamroleset promote role` '
         'before you can enable promotion role prerequisite.'
         '').format(ctx.prefix)
      )
      return
    ...
    if target_state:
      await ctx.send(
        ('Done. Promote role prerequisite is now enabled. Only members '
         'with the role `{}` will be given the streaming role.'
         '').format(prereq_role.name)
      )
    else:
      await ctx.send(
        ('Done. Promote role prerequisite is now disabled. All members '
         'who are streaming will be given the streaming role.'
         '')
      )
  ...
  async def streamroleset_promote_role(self, ctx: commands.Context, *, role: discord.Role):
    ...
    members with this role will be given the streaming role.
    '''
    ...
    await ctx.send(
      ('Done. Only members with the role `{}` '
       'will be given the streaming role.'
       '').format(role.name)
    )
  ...
  async def streamroleset_promote_lax(self, ctx: commands.Context, on_off: bool = None):
    ...
    role above it in the hierarchy will be given the streaming
    role. If it is off, only members with exactly the prerequisite
    ...
    if target_state:
      await ctx.send(
        ('Lax promotion is now on. If promotion prerequisite is '
         'enabled, any member with the prerequisite role or any role '
         'above it in the hierarchy will be given the streaming role.'
         '')
      )
    else:
      await ctx.send(
        ('Lax promotion is now off. If promotion prerequisite is '
         'enabled, only members with exactly the prerequisite '
         'role will be given the streaming role.'
         '')
      )
  ...
  async def on_member_update(self, before: discord.Member, after: discord.Member):
    ...
    if config['enabled'] and config['role'] is not None:
      ...
      if streaming_role is None:
        log.error(
          ('Failed to find streaming role with ID {} '
           '(server ID {}). This probably means that '
      ...
      if activity is None \
        ...
        try:
          ...
          log.warning(
            ('Failed to remove role ID {} from member ID {} '
             '(server ID {}): insufficient permissions'
      ...
      # and if allowed by promotion settings
        ...
        if self.__can_promote(after, config):
          try:
            ...
            log.warning(
              ('Failed to add role ID {} to member ID {} '
               '(server ID {}): insufficient permissions'
               '').format(streaming_role.id, after.id, guild.id)
            )
          except:
  ...
  def __can_promote(self, member: discord.Member, config: dict) -> bool:
    '''Returns a boolean indicating whether member can be given
    the streaming role based on the promotion rules defined in
    config.
    '''
  ...
  def __has_role_above(self, member: discord.Member, role: discord.Role) -> bool:
    '''Returns a boolean indicating whether member has a role
    which is higher than role in the hierarchy.'''
