...
class KZ:
    ...
    async def _addmap(self, ctx: commands.Context, map_id_or_url: str):
        ...
        if new_map is not None:
            await self.bot.reply(cf.info("New map `{}` added. You must restart the"
                                         " server for the mapcycle change to take "
                                         "effect. Would you like to restart it "
                                         "now? (yes/no)".format(new_map)))
